...
query_select = '''SELECT A.ID, A.TICKER, MAX(B.DT_COTACAO) AS DT_COTACAO
FROM tbl_ativos_listados_b3 AS A
JOIN tbl_cotacao_listados_b3 AS B
ON A.ID = B.ID_ATIVO
GROUP BY A.ID, A.TICKER'''

df_ativos = SQL_CONN_ATIVO.select_data(query_select, return_as_dataframe=True)
df_ativos.columns = ['ID_ATIVO', 'TICKER', 'DT_COTACAO']

# Carregar os dados
tickers = list(df_ativos['TICKER'].unique())
tickers = [f"{ticker}.SA" for ticker in tickers]

cotacoes = yf.download(tickers=tickers, start=df_ativos['DT_COTACAO'].min(), end=dt.datetime.today().strftime('%Y-%m-%d'), threads=True, group_by="ticker")
